...
- Optional integration with **FileBrowser Quantum** and **Nextcloud** as sources for external photo libraries.
...
External libraries managed from StartOS can only be sourced from FileBrowser Quantum or Nextcloud — for anything else, use Immich's own library settings. Connecting a source makes that service's whole volume readable by every Immich administrator, so only connect sources you're comfortable exposing to your Immich admins. Manage External Libraries and Immich's own library settings are two views of the same libraries, so a change in either place shows up in the other; removing a library from the StartOS action deletes it from Immich outright.
